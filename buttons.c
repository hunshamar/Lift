...
void buttons_poll(void){
    for (int floor = 0; floor < 3; floor++){ // Check all call up order buttons
        if (elev_get_button_signal(BUTTON_CALL_UP, floor)){
            orders_add(floor, DIR_UP);
            elev_set_button_lamp(BUTTON_CALL_UP, floor, 1);
        }
    }
    for (int floor = 1; floor < 4; floor++){ // Check all call down order buttons
        if (elev_get_button_signal(BUTTON_CALL_DOWN, floor)){
            orders_add(floor, DIR_DOWN);
            elev_set_button_lamp(BUTTON_CALL_DOWN, floor, 1);
        }
    }
    for (int floor = 0; floor < 4; floor++){ // Check all command order buttons
